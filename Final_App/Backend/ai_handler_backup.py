...
class AIHandler:
    ...
    def __init__(self):
        ...
        if self.ollama_url and self.ollama_model_name:
            try:
                # Check if Ollama is reachable
                r = requests.get(f"{self.ollama_url}/api/tags", timeout=5)
                models = [m["name"] for m in r.json().get("models", [])]
                if any(self.ollama_model_name in m for m in models):
                    self.ollama_ready = True
                    print(f"✓ Ollama initialized  – model: {self.ollama_model_name}")
                else:
                    print(f"⚠ Ollama connected but model '{self.ollama_model_name}' not found. Available: {models}")
            except Exception as e:
                print(f"⚠ Ollama init failed: {e}")

        if not self.gemini_available and not self.openai_ready and not self.ollama_ready:
            print(
                "⚠ No LLM provider is available! "
                "Set GEMINI_API_KEY, OLLAMA_BASE_URL, or OPENAI_API_KEY in .env"
            )
    ...
    def generate_response(
        ...
        # 0️⃣  Try Ollama (prioritized if configured)
    ...
    def _call_ollama(
        ...
        """Call local Ollama instance and return the response text."""
        payload = {
