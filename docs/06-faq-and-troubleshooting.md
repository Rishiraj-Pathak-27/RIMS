...
### Q8: What exact data is generated by the live data generator pipeline?
**Answer**: On every 10-second tick, `data_generator.py` produces:
1. **Order Telemetry (25 Features)**: Customer segment (`Consumer`, `Corporate`, `Home Office`), market (`LATAM`, `Europe`, `USCA`, `Asia Pacific`, `Africa`), shipping mode (`Standard Class`, `Second Class`, `First Class`, `Same Day`), sales volume ($\$15 - \$4500$), quantities ($1 - 30$), profit margins ($2\% - 45\%$), lead times ($0 - 18$ days), shipping costs ($\$2 - \$60$), processing days ($1 - 6$), and **controlled defect rate spikes ($5.0\% - 18.0\%$) in ~10% of ticks** to trigger Isolation Forest anomalies.
2. **Demand Lag Features**: Autoregressive lag features (`lag_1`, `lag_2`, `lag_3`, `month`) drawn from realistic historical demand pools with $\pm 8\%$ random jitter.
